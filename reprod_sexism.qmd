...
The histogram above, which depicts intended votes for Vox (coded as 1 for "Yes" and 0 for "No"), shows a binary distribution, with a significantly higher frequency of 0s (3,491 for non-Vox voters) compared to 1s (258 for Vox voters). This refined approach provides a more precise assessment of Vox's appeal during its period of rising popularity.

### Data Cleaning and Missing Data

-   Nonresponses and intended nonvoters were coded as zero.

-   In order to assess the model for cross validation

-   None of these missing data were used to fit the original model in Table 1 of the paper
...
```{# {r}
# finding out how many NAs in the data set for each variable in 2020

# # Subset the data for the year 2020
# data_2020 <- data[data$year == 2020, ]
# 
# # Count the number of NAs in the vim_vox column
# num_na <- sum(is.na(data_2020$vim_vox))
# 
# # Print the result
# print(num_na)
# 
...
# covariates <- c("vim_vox", "female", "age", "edu3", "dhincome_all", 
#                 "livingpartner", "intpol", "authoritarian", "ideol", 
#                 "nativism", "orgterr", "pop6amz", "msexism")
# 
# # Count the number of NAs for each covariate
# na_counts <- colSums(is.na(data_2020[, covariates]))
# 
# # Print the result
# print(na_counts)
```
...
t1m2 <- glm(vim_vox ~ female + age + factor(edu3) + dhincome_all +  
             ...
             data = data[data$year == 2020, ],  # Subset the data for 2020
             family = binomial(link = "logit"),
             x = TRUE,  # Include the model matrix in the output
             y = TRUE)  # Include the response vector in the output


new.df <-t1m2$x
response_values <- t1m2$y
```
...
t1prob2 <- glm(vim_vox ~ female + age + factor(edu3) + dhincome_all +  
              livingpartner + intpol + authoritarian + ideol + 
              nativism + orgterr + pop6amz + msexism,
              data = new.df,  # Subset the data for 2020
...
```{r}
#getting rid of income in all the models - see if ROC works and CV works 

t1m2_rmin <- glm(vim_vox ~ female + age + factor(edu3) + 
               livingpartner + intpol + authoritarian + ideol + 
               nativism + orgterr + pop6amz + msexism,
...
t1prob2_rmin <- glm(vim_vox ~ female + age + factor(edu3) +  
               livingpartner + intpol + authoritarian + ideol + 
               nativism + orgterr + pop6amz + msexism,
             data = data[data$year == 2020, ],  # Subset the data for 2020
             family = binomial(link = "probit"))
...
# Filter data for 2020 and fit the logistic regression model
t1cloglog2_rmin <- glm(vim_vox ~ female + age + factor(edu3) +  
               livingpartner + intpol + authoritarian + ideol + 
               nativism + orgterr + pop6amz + msexism,
             data = data[data$year == 2020, ],  # Subset the data for 2020
             family = binomial(link = "cloglog"))

stargazer(t1m2_rmin, t1prob2_rmin, t1cloglog2_rmin, type = "text", 
...
#ROC Curve - Logit, Probit, Clog Log for Year 2020

I removed the variable dhincome - household income because it contained 36 missing values (NA's). I wanted to retain as many observations as possible, thus I opted to delete the variable so that the model can be trained on as many observations as possible.

```{r}
library(pROC)

# Generate predicted probabilities for the logistic regression model (t1m2)
pred_logit <- predict(t1m2_rmin, type = "response")

# Generate predicted probabilities for the probit model (t1prob2)
pred_probit <- predict(t1prob2_rmin, type = "response")

# Generate predicted probabilities for the cloglog model (t1cloglog2)
pred_cloglog <- predict(t1cloglog2_rmin, type = "response")

length(data[data$year == 2020, ]$vim_vox)
length(pred_logit)
```
...
```{r}
t1simp <- glm(vim_vox ~ authoritarian + ideol + 
                        nativism + orgterr + pop6amz + msexism,
                      data[data$year == 2020, ], family = binomial(link = "logit"))
summary(t1simp)
```

Because my simpler model ran, I hypothesized that there must be a variable in the more general model that was stopping the cross validation from running. I found that I had 36 missing values for income in year 2020, so in order to preserve as much data as possible, I removed this variable as it was also not significant in the authors' original table. I
...
for (i in 1:k) {
  ...
  # Fit t1m2 model on training data - REMOVED INCOME VARIABLE
  model_t1m2 <- glm(vim_vox ~ female + age + factor(edu3) +  
                      livingpartner + intpol + authoritarian + ideol + 
                      nativism + orgterr + pop6amz + msexism,
                    data = train_data, family = binomial(link = "logit"))
...
```{r}
# List all columns in the data_combo dataset
column_names <- names(data_combo)

# Print the column names
print(column_names)

# Access the vim_vox column
vim_vox_column <- data_combo[["vim_vox"]]

# Print the vim_vox column
print(vim_vox_column)

# Count the number of 1's in the vim_vox column
count_ones <- sum(data_combo$vim_vox == 1, na.rm = TRUE)
...
for (i in 1:k) {
  ...
  # Fit models on training data # removed dhincome
